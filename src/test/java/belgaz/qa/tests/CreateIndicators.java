...
public class CreateIndicators extends TestBase {
    ...
    public void test2CreateIndicators() throws Exception {
        ...
        application.inputSearchName("gs_name29050");

        application.clickOnEnter();
        application.openFoundClient("gs_name2905046755");
...
//---------------------------------------------------------------------------------------------------------------------
        ...
        application.getSignalHelper().clickAddSignal();
        application.getSignalHelper().clickOnSearchSignal();
        //Привлечение к уголовной ответственности руководителей, учредителей, акционеров, бенефициарных собственников  - (1-й сигнал для 4-го индикатора)
        application.getSignalHelper().chosenSignal1Indicotor4();
        application.getSignalHelper().fillFromSignal1Indicotor4(new Signal1Indicotor4Data("ФИО связанного лица","Примечание"));


        application.driver.findElement(By.xpath("(//input[@type='text'])[11]")).click();
        application.getSignalHelper().LocalDate();
